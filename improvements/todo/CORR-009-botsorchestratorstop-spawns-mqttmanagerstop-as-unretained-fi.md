...
- [ ] mqtt_manager.stop() is awaited (or its task is retained and awaited) during orchestrator shutdown
- [ ] MQTT connection and subscriptions are reliably torn down on shutdown with no leaked task warnings
- [ ] No 'Task was destroyed but it is pending' or 'no running event loop' errors during shutdown
