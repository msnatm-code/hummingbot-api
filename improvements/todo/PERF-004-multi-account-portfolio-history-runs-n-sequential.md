...
- [ ] Multi-account history no longer awaits each account query strictly in series
- [ ] Returned data is ordered by timestamp desc and limited correctly across all requested accounts
- [ ] Pagination cursor produces non-overlapping pages across accounts
- [ ] Endpoint response shape is unchanged for existing single/all-account callers
