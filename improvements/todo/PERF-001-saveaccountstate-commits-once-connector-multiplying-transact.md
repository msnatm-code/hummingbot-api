...
- [ ] save_account_state no longer calls session.commit(); it only flushes to get the id
- [ ] dump_account_state performs exactly one commit per snapshot regardless of account/connector count
- [ ] A snapshot with multiple accounts/connectors persists all token_states atomically and reads back identically to before
- [ ] Existing tests in test/ still pass
